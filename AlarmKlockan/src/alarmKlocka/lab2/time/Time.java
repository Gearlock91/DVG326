...
public class Time implements TimeType
  ...
  public Time(int hour, int minute, int second) throws BadTimeFormat
    {
	  if((0 <= hour && hour <= 23) && ( 0 <= minute && minute <= 59) && (0 <= second && second <= 59)) {
		  ...
		  throw new BadTimeFormat("Bad time format in Time(hour,minute,seconds)");
	  }
    }

  public Time(int day, int hour, int minute, int second) throws BadTimeFormat
    {
    this(hour,minute,second);
    	if(0 <= day && day <= 6)
    		setDay(day);
    	else {
    		throw new BadTimeFormat("Bad time format in Time(days)");
    	}
    }
